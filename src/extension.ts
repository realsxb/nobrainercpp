...
return [
        ...
        " * Github Repository: https://github.com/realsxb/nobrainercpp",
        " * Feedback email: nebulazeyv@nebulazeyv.com, 23182625@buaa.edu.cn",
        " * The author was tired of manually configuring the C/C++ environment in VSCode,",
        " * and decided to write a plugin to generate configuration files with one click for a 'set and forget' experience.",
        " * This plugin prioritizes installed toolchains in PATH. If missing, it uses the embedded toolchain.",
        " * Zero dependencies, plug-and-play. You can use this plugin to compile C/C++ even on a fresh VSCode installation!",
        " * Hope this helps as many BUAAers (and everyone else) as possible!",
        " * ------------------------------------------------------------------",
        " */",
        "" // 空一行
    ].join('\n');
...
const logo = `
 ...
 |_| \\_|\\___/|____/|_|  \\__,_|_|_| |_|\\___|_|    
    ...
    >>> Usage ->
    >>> Just click the "Run" button (arrow icon) in the top right to start debugging C/C++ without any manual configuration.
    >>> Embedded Debugger Usage: Please press the Green Triangle ▶ (Continue/F5) on the debug toolbar to jump between breakpoints.
    >>> Embedded Debugger Note: Pressing 'Step Over' before the first breakpoint or after the last one will enter assembly files. Press Green Triangle ▶ (Continue/F5) to exit.
    `;
...
// ================== 新增：检测微软扩展 ==================
    ...
    // 提示逻辑优化
    const toolchainName = toolchain.toolchain === 'embedded' ? "Built-in Toolchain" : toolchain.toolchain;
    
    if (toolchain.toolchain !== 'embedded' && !hasMsExtension) {
        // 情况：有 GCC 但没微软插件
        vscode.window.showWarningMessage(
            `Detected local compiler (${toolchainName}), but Microsoft C/C++ extension is not found. Using NBCpp built-in debugger.`,
            "Install Microsoft Extension"
        ).then(selection => {
            if (selection === "Install Microsoft Extension") {
                vscode.commands.executeCommand('extension.open', 'ms-vscode.cpptools');
            }
        });
    } else {
        vscode.window.showInformationMessage(`NoBrainerCpp: Auto-configuration complete (${toolchainName})`);
    }
    // =======================================================
    // 3. 写入配置文件
